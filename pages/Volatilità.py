...
def calculate_volatility_metrics(df: pd.DataFrame, fund_name: str) -> pd.DataFrame:
    ...
    try:
        ...
        return df_fund[['Date', 'Quota', 'Volatilità_1Y', 'Drawdown']]
...
def calculate_summary_stats(df_metrics: pd.DataFrame) -> dict:
    ...
    return {
        ...
        'dd_current': dd_data.iloc[-1] if len(dd_data) > 0 else np.nan,
        ...
        for i, fund in enumerate(available_funds):
            ...
            if not metrics_df.empty:
                ...
                summary_data.append({
                    ...
                    'Volatilità Media Storica (%)': stats['vol_mean'],
                    'Volatilità Massima Storica (%)': stats['vol_max'],
                    'Drawdown Corrente (%)': stats['dd_current'],
    ...
    if not summary_data:
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Volatilità Media Storica (%)": st.column_config.TextColumn("Vol. Media", width="small"),
                "Volatilità Massima Storica (%)": st.column_config.TextColumn("Vol. Max", width="small"),
                "Drawdown Corrente (%)": st.column_config.TextColumn("DD Corrente", width="small"),
        ...
        with col_chart1:
            ...
            fig_vol.update_layout(
                title="Volatilità 1Y Attuale per Fondo",
                xaxis_title="Fondo",
